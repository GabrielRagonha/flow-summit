          ...
          <div class="h-full  w-full px-4 flex flex-col justify-between">
            ...
            <div>
              <ion-card class="my-2 mx-0">
                ...
                <ion-card-content>
                  <img src="https://picsum.photos/1000" v-bind:alt="'Imagem do evento ' + event?.name" />
                </ion-card-content>
            ...
            <div class="mb-5">
              <button id="confirm-btn" @click.prevent="handleConfirm()" fill="outline" color="dark"
                class="w-full text-white h-14 font-bold bg-[#F97316] rounded-lg" type="submit">
                Confirmar presença
              </button>
            </div>
...
onMounted(async () => {
  try {
    ...
    if (scheduleUser.find(schedule => schedule.idEvent === event.value.idEvent)) {
